...
$s \gets 0$, $t \gets 0$ \;
$s' \gets n - i + 1$, $t' \gets {\cal C}_{s'} - 1$ \;

\While{$D[p].v < x$} {
  $t \gets {\cal T}(D[p].s, D[p].t, s, t)$ \;
    \label{alg:cartesian-encode-online-pop-c1}
  $s \gets s + D[p].s+1$ \; 
    \label{alg:cartesian-encode-online-pop-c2}
  $t' \gets {\cal T}(D[p].s, D[p].t, s', t')$ \;
    \label{alg:cartesian-encode-online-pop-virtual-c1}
  $s' \gets s' + D[p].s+1$ \;
    \label{alg:cartesian-encode-online-pop-virtual-c2}
  $p \gets p - 1$ \;
}
$p \gets p + 1$ \;
