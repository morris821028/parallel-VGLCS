...
We increase the number of input elements ($N$ in
Table~\ref{tlb:CORMQ}) to verify the scalability of
Algorithm~\ref{alg:parallel-sparse-table}.  % explain the scalability

The maximum length of interval query ($L$ in Table~\ref{tlb:CORMQ})
affects the performance of accessing different levels in the sparse
table.

The number of levels in the compressed sparse table is smaller than
that of the sparse table.

Therefore, when $L$ is small, it is very likely that the interval
query can be processed in the same level of the sparse table.

This improves performance of cache temporal locality for compressed
sparse table because multiple data store in compressed block.


Oppositely, when $L$ is large, data sharing decreases the efficiency
during integrating the maximum value by the definition of recursion.
