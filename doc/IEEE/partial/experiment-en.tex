...
Cartesian index computation, which the compressed sparse table does
not.  When the probability $q$ is close to 1, $O(1)$ sparse table will
require less instructions on computing Cartesian index, so its
performance will be close to that of the the compressed sparse table.
...
table, but not of $O(1)$ sparse table.  Note that in
Algorithm~\ref{XXX} the block size $s$ is set to 16, which is the
maximum number of times the loop in Algorithm~\ref{XXX} will iterate
for an query.  Therefore the query time will increase when $L$
increases.  On the other hand, the $O(1)$ sparse table answer a query
by a single lookup into the LCA table, so its performance is not
affected by $L$.
