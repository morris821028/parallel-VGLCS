...
Disjoint set: We implement amortized time $o(\alpha(n))$ by path
compression and rank strategies.  If occupy the equivalence in above
strategies, tend to use {\sc Rem} strategy.  In order to solve ISMQ
problem, using the stack to store the indices of maximum value node
...
Compressed sparse table: The insert operation consume amortized $O(1)$
time.  The query operation consumes $O(s)$ time, which $s$ is the size
of the block.  Here, the program implement by the fixed size $s = 16$.
We can maintain extra the prefix and suffix maximum in block to reduce
the time complexity $O(s)$ to $O(1)$ in most queries.  It is introduced
in the Section~\ref{sec:parallelRMQ}
