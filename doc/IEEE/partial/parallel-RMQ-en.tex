...
\ifdefined\MasterThesis
\section{Blocked Sparse Table} 
\else
\subsection{Blocked Sparse Table} 
\fi
\label{sec:blocked-sparse-table}
...
\ifdefined\MasterThesis
\section{Right-most-pops Encoding}
\else
...
The key idea of right-most-pops encoding is that we can use $t_i$'s to
...
as in Fischer's algorithm. The overall time complexity of the parallel
version of our VGLCS algorithm becomes $O(n^2 \log{n} / p + n \log
n)$.  Note that the $\log n$ comes from the block size $s = O(\log n)$
