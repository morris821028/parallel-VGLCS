...
blocks in a on-demand manner.  Whne the algorithm finds that the
...
the number of blocks times, and causes cahche misses.
...
can answer ranged maximum queries directly from examing this binary
string.  However, this opertion requires counting the number of 1's
between the last two 0's, therefore it is hard to implement
efficiently in a modern computer.

We propose a new enconding for blocks.  We will use another
representation, called {\em compressed sparse table}, instead of the
binary string to imporve perfromance.  The compressed sparse table is
inspired by Fischer's sparse table and Cartesian tree.

We replace lookup operation to naive operation.
