\Abstract{ The longest common subsequence problem with variable gapped
contraints (VGLCS) is used in genes, molecular biology.  We can find
$O(nm \alpha(n))$ solution in previous study, which use the efficient
incremental suffix maximum query (ISMQ). ISMQ supports append a value
to array and get the suffix maximum value in averge $O(\alpha(n))$
...
\smallskip \noindent \textbf{Keywords.} range minimum query,
